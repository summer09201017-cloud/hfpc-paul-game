...
**Multi-journey is now wired.** `useGame.js` holds a `JOURNEYS` array — currently `paul` (journey1, 20 tiles), `paul2` (journey2, into Europe, 20 tiles), `paul3` (journey3, Ephesus years → Jerusalem, 20 tiles), `jonah` (journey-jonah, 20 tiles), **`exodus` (journey-exodus, 22 tiles, 2026-06-12)** and **`daniel` (journey-daniel, 20 tiles, 2026-06-12)**. **`paul4` (journey4 海路到羅馬, Acts 27–28, 20 tiles) lives on branch `feat/journey4-rome`** (2026-06-12 晚, branched off `feat/daniel-exodus` — merge that branch first); it completes the relay `paul1 → paul2 → paul3 → paul4` and its review sheet is in that branch's `docs/`. Each entry can carry `nextKey` (宣教接力: finishing a journey offers "continue to the next one", carrying name/points/gifts; companions reset to the new journey's `startCompanions`). `SetupScreen` shows a journey picker. To add another:
...
4. `npm run test:selfplay` already loops over all journey files — add the new one to its `JOURNEYS` list too. `npm run validate` needs no change — with no args it auto-validates every `src/data/journey*.json`.
