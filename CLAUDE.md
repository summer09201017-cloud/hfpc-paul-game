...
> - 🔒 鐵則(2026-07-10 使用者拍板):**所有 Canvas 人物預設都要有眼睛、表情、嘴巴**——投手/打者/NPC/對手全部,遠景小人也要(點眼+短嘴線),表情跟狀態反應(專注/驚訝/開心/懊惱)。活範例:baseball 投手與打者。
...
### 引擎總覽(2026-07-09 對賬;50 個引擎一行一個,防「程式有、文件沒提」)
...
| `gideon/` | `?demo=gideon` | 士 6:25-27 基甸拆祭壇 | **新類型⑪打磚塊**:夜色+火把光,滑鼠跟隨/←→/拖曳移木槓彈石球拆巴力壇石塊,頂上木偶 2 下砍倒;球掉出=僕人悄悄撿回不扣命,永不會輸;拆完必接「築真壇」一幕(士 6:26,先拆假的才立真的);⚠ **絕不可換皮成耶利哥**(牆是神拆的不是人砸的);年齡三檔=球速/槓寬/磚層(3/4/5)×**壇石硬度(07-09 牧者點名加難:幼 2/童 3/青 4 下才全碎,裂痕漸進+未碎掉小碎屑;卷軸掉落率 0.22→0.3、上限 3 補節奏;木偶維持 2 下)**;✅ 牧者已過審(2026-07-07,PR #73 併入)、大廳卡已亮;07-09 牧者拍板加料(✅ 已過審 PR #80 併入):**應許卷軸掉落**——道具名=士 6 神對基甸的原話(6:12 同在=多一顆球/6:14 能力=木槓加寬 8s/6:16 擊打=**石鑿**連發 4s,工匠鑿石非開槍);沒接到落地淡去不懲罰;滑鼠不按住也跟隨 |
...
| `football/` | `?demo=football` | (無經文)世界盃足球賽・實況版 | **憫安休閒第三彈(即時操作,牧者點名「真的運球與踢球,不要撞球方式」)**:直接操控藍隊 10 號——WASD/方向鍵跑位、貼身自動帶球(球黏腳前)、**按住空白鍵蓄力放開踢**(觸控:按住往那跑、帶球點一下往那踢);隊友對手全即時 AI(追球/帶球推進/射門/守門員沿門線撲救+解圍)、抄截有 0.5s 保護防瞬搶;🤖 AI 三檔速度/👥 雙人同機(P2=方向鍵+Enter);溫柔規則 won:true;與回合彈射版(soccer)是同題材兩玩法;PR #83;07-09 加 **Q/Shift 切換球員+隊友被堵回傳**;★07-10 使用者拍板大改(PR #87):**AI 再降弱**(aiSpd 68/98/126+射門醞釀 0.9/0.55/0.3s+射門變軟)、**圓盤改小人**(有眼睛表情嘴巴,瞳孔看球、走路擺腿、進球隊笑/被進隊扁嘴;碰撞仍是圓零物理改動)、**不限時先進 N 球獲勝**(N=玩家輸入 1~10,步進器+點數字鍵入)、輸入卡鍵三修(key 小寫化/Q 防連發+0.3s 冷卻/失焦清鍵);07-10 再弱化(PR #88,使用者回報「AI 太會防守球門踢不進」):**守門員獨立慢速 keepSpd 34/62/96**(不再用全隊 aiSpd 貼死射線,幼檔門將幾乎站樁,大力射角度就進)+疲勞保底提早(45s 沒進球 ×0.6);**每隊幾人玩家輸入(2~7,預設 4;步進器與先進幾球同列;>7 場地會擠成人牆故釘上限;7 人站位表 F_XS/F_YS)** |
| `hoopshot/` | `?demo=hoopshot` | (無經文)投籃大賽 | **憫安休閒第四彈(側視投籃,goalkick 姊妹作)**:輪流出手——按住蓄力,力道環有「綠色甜蜜區」,區內放開=空心入網;差一點=打框看運氣(幼 60%/童青 40% 彈進)、差很多=空氣球;出手點越換越遠,三分線外進球算 3 分;**每人幾球玩家輸入(1~30,預設 8;步進器+點數字鍵入,07-10 PR #87)**分高者勝;🤖 對戰阿福教練/👥 雙人同機輪流;**射手有臉(07-10)**:眼睛瞄籃時看上方、進球笑瞇眼、沒進扁嘴八字眉;溫柔規則 won:true;無經文免送審 |
| `basketball/` | `?demo=basketball` | (無經文)世界盃籃球賽 | **憫安休閒第五彈(football 姊妹作,即時運球+投籃)**:WASD 跑位、貼身自動運球、按住空白鍵蓄力放開出手——★蓄力淺=傳球(平傳可被抄)/深=投籃(拋物線奔籃框,空中不可抄,甜蜜區=空心);打框彈出=籃板球大家搶;三分線外 3 分;先到 15 分或時間到比分高者勝;🤖 AI/👥 雙人同機;溫柔規則 won:true;07-09 加 **Q/Shift 切換球員+隊友被堵回傳**;★07-10 使用者回報「重複切換/卡住不能動不能傳不能投」已修(PR #87,三因:Q keydown 連發沒擋 e.repeat、Shift 大小寫卡鍵 _keys.w 永遠 true、失焦漏 keyup——修=key 小寫化+防連發 0.3s 冷卻+blur 清鍵,football 同批修)+**圓盤改小人有臉**(同 football 畫法,碰撞不變);07-10 使用者點名(PR #88):**每隊幾人玩家輸入(2~7,預設 4;步進器+點數字鍵入,7 人站位表防出界)** |
| `baseball/` | `?demo=baseball` | (無經文)棒球打擊王 | **憫安休閒第六彈(07-10 大改:主審視角+好壞球,三模式)**:球從遠方投手丘朝鏡頭飛來由小變大(殘影);虛線好球帶分好壞球——壞球可不揮、四壞=🚶保送+1、B/S 燈號、三好=溫柔換打席(無出局);時機窗以秒計(幼0.16/童0.11/青0.085);界外球真的打飛出邊線;🤖 打擊練習=阿福投 10 球你打/🎯 **投球挑戰=你投阿福打**(AI 打者依年齡檔選球出棒,壓低阿福得分拿星)/👥 投打對決=P1 投 P2 打攻守交換;★07-10 加(PR #88):**五種球路**(快速/慢速/曲球🌜/滑球⚡晚破/伸卡⤵️晚沉;幼1童3青5種,彎的是途中軌跡、終點仍是選定落點)+**投手瞄準加左右**(W/S×A/D 選 5×5 網格:**好球帶畫成 3×3 九宮格、內九格任選+外圈引誘壞球位**,Q/E 換球種;投球挑戰亮選中格+落點準星、對決不畫防洩底);**pitch/2p 球數玩家輸入(1~30,預設 6;步進器+點數字鍵入)**;投手打者都有眼睛表情嘴巴(★人物臉部鐵則活範例);溫柔規則 won:true;無經文免送審(PR #85 初版、#86 大改) |
| `billiards/` | `?demo=billiards` | (無經文)花式撞球 | **憫安休閒第七彈(真撞球,07-10 使用者點名「真正的撞球」)**:頂視球桌+六袋口+真物理入袋——herd 撞球物理的「轉正版」(圓-圓彈性碰撞/顆星反彈 0.88/摩擦 0.38^dt 大力桿約 4~5 秒停/子步進防穿透/袋口附近不反彈讓球掉得進);簡化雙色制:藍組 vs 紅組各 N 顆(**每組幾球玩家輸入 2~7**),清光自己顏色獲勝;進自己的=續桿、進對方的=算對方不罰、白球洗袋=擺回開球點換人(溫柔無罰);拉桿式操作(按住往後拉=蓄力,放開出桿)+瞄準提示三檔(幼=瞄準線+假想球+目標球走向/童=中/青=短線)+球桿手繪;🤖 阿福 AI(假想球點×袋口遍歷+路徑遮擋檢查+角度可行性,年齡誤差 0.11/0.055/0.025 rad)/👥 雙人同機;溫柔規則 won:true;無經文免送審 |
