...
**In-repo collect/recover engine (2026-06-15, `src/minigames/elijah/`, branch `feat/elijah-action`):** a real-time **collect-to-recover** auto-runner — 盼望·以利亞重得力 (灰心的以利亞在曠野撿天使預備的餅🍞水💧恢復體力，走到何烈山，王上 19:5–8). This is the **action version** of the 盼望 theme (the card version `elijah` in `cards/specs.js` is unchanged and still lives at `?demo=elijah`). It is the concrete first cut of roadmap 待辦 f「收集機制換皮」, done as a **self-contained engine** rather than an upstream Jonah `spawner` change — so it needs **zero coordination with the Jonah `art-kenney-sprites` work and causes no fork drift** (`sync:jonah` never touches it). A station carries `minigame: { engine: 'elijah', winPoints, label }`; `MiniGameModal` boots `ElijahGame` (same `boot()`/`destroy()`/`onComplete({won,score,level:'elijah'})` contract as Sling/Jonah). Files mirror the sling layout: `config.js` (tunables — `STAMINA` start/drain/bread/water, `FAINT`, `BOOST`, `NEED_MEAL`), `spawner.js` (**the reskin**: 🍞/💧/🥖 `recover` collectibles + gentle 曠野 hazards, copied & themed from Jonah's `spawner.js`), `game.js` (state machine intro→playing→win/**faint**), `renderer.js` (wilderness→Horeb backdrop, stamina bar, broom-tree🌳+angel👼, reuses Jonah's `_prophet`/`_buildings`/`roundRect`/`_emoji` vectors), `player.js`/`input.js`/`audio.js`/`content.js`. **Feel = gentle-but-tense:** stamina drains as you run, bread/water refill it; **stamina 0 ≠ failure** — Elijah slumps (`faint`, 王上 19:7「起來吃吧，因為你當走的路甚遠」) and God lifts him up again (auto-refill, resume in place; no life lost, no full restart). Dev preview: `?demo=elijah-action` (mounts `ElijahDemo`; `window.__elijah` hook for screenshot tests); Playwright capture script `scripts/elijah-shots.mjs`, double-click launcher `play-elijah-action.bat`. **Not yet wired to a journey** — content (和合本 verbatim, pastoral) is **AI-drafted and awaits the pastor's review before going live** (same gate as the 盼望 card). The reusable recover/collect pattern is meant for future 收集 levels (manna 嗎哪 / Ruth gleaning 拾穗 / Daniel 王膳); blueprint = skill `collect-recover-minigame`.
