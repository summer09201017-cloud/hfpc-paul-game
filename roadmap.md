...
> - #79 💎 gems 兩加料(🧠 記憶挑戰=亮 10 秒遮住+再看一眼不限次/❓ 名字的意思=12 題全出自和合本括號註 創 29-30;35:18)
>   +🌾 拾穗的路得(⑰第二實作,`?demo=glean`,★斜線實驗版:橫直斜都算一排=恩典多給;兩拍慢節奏+捆繩;
>   平衡:補位軟迴避——初版一手雪崩 84 穗)+⚽ goalkick 進網深處/守門員抱球/直射蓄不滿力 bug 修。
> - #80 ⚒️ gideon 應許卷軸道具(士 6:12 同在=多球/6:14 能力=加寬/6:16 擊打=石鑿連發——神的話壯膽非咒語換獎)
>   +滑鼠不按住也跟隨+🐏 雅各的斑點羊(撞球⑯雙欄分類,`?demo=flock`,kind `pool`)+docs/足球全場對戰-設計.md。
> - #81 🏆 世界盃足球賽(`?demo=soccer`,回合彈射全場,AI 阿福教練擬人頭像+雙人同機 PK,無經文憫安合輯 kind `sports`)。
> - 🆕 npm test 補 **check-jsx 守門**(scripts/check-jsx.mjs,esbuild 快掃全部 .jsx——機器化 07-08 的 jsx 建置盲區血淚,負面測試驗過)。
> - 現況:**46 引擎/48 bat/manifest 62 句**;大廳 49 卡+憫安合輯 39 關(sw v35)。
> - 牧者新定案:拾穗類/收取類消消樂**預設開斜線**(skill match3-swap 已改,manna 不回改);消消樂連鎖要兩拍慢節奏。
>
> **🔨 真正待做(依拍板/CP 序):**
> 1. 涵蓋地圖缺口:①大衛迎約櫃(撒下 6,miriam 換皮,半天)②以利亞迦密山(王上 18,cards 加 key)
>    ③腓利與太監(徒 8)④喇合(書 2,潛行)⑤以斯帖。
> 2. 撞球候選下一個:🐟 約 21 滿網之魚(「網卻沒有破」,herd 換皮半天)。
> 3. 金鑰債 /key-migrate(兩台機,人工輪換)——最老的安全待辦。
> 4. war-games 退役(等使用者定)。

> ## ⭐ 對齊現況:2026-07-08 晚(已過時——當時待審的 #75-77 均已併)
>
> **🔨 三個新關+temple 手感升級(feat/fragments-fruits-gems,PR 待牧者審——過審前不併 main、不進大廳卡):**
> - 🧺 五餅二魚・收拾零碎(彈珠配對⑭第二實作,?demo=fragments):約 6:11-13;零碎五種,配對=收進十二個籃子
>   (免得有糟蹋的,非消失);籃子逐一裝滿,結局=裝滿十二個籃子(6:13)。
> - 🍇 聖靈果子・結果子(⑭第三實作,?demo=fruits):加 5:22-23+約 15:5;果子帶字(仁愛/喜樂/和平/忍耐/恩慈),
>   配對=結到樹上;★守法:果子是聖靈結的,教導句約 15:5 把功勞還給主。
> - 💎 大祭司胸牌・寶石歸位(**新類型⑮歸位配對**,?demo=gems):出 28:17-21+29;石名四行完全照和合本、
>   支派照生來次序;幼 6 塊看顏色/童槽標石名/青對照經文卡;放錯溫柔搖頭;完成=名字帶在胸前+金光。
> - 🧱 temple 手感升級(牧者已過審關的玩法 QoL,文案零變動):⏸ 暫停(P/Esc/鈕+pause/resume 嵌入契約)、
>   「暫存(C)」交換石頭(每次落定前限一次)、青檔預覽 0→1。
> - 全鏈:cuv 逐字 5 段(約 6:11-13/加 5:22-23/約 15:5/出 28:17-21/28:29)→ 引擎+Demo+路由+bat(42 支)→
>   mp3 烤畢(manifest 58 句,新產 3→重跑 0)→ npm test 158 綠 → Playwright 截圖 → PR 待審。**40 引擎、42 bat**。

> ## ⭐ 對齊現況:2026-07-07 晚(接手的 AI 先讀這段)
>
> **✅ 四個新關全鏈完成(牧者已過審 2026-07-08、PR #74 併入 main、A 站驗畢、大廳四卡點亮):**
> - 💼 好管家(**經營管理②**,`?demo=steward`,play-steward.bat):太 25:14-30+林前 4:2;每季分銀袋到市集/坑;
>   ★星等按忠心不按金額(五千的和二千的得同一句稱讚=幼童檔同權);埋太多=溫柔教導,永不會輸。
> - 🪙 失錢找物(**新類型⑫找物/找碴**,`?demo=lostcoin`,play-lostcoin.bat):路 15:8-10;油燈跟手指照暗屋細細地找;
>   ★沒有時間失敗——直到找著;青檔要「打掃」家具;找著=鄰舍一同歡喜+天上的歡喜(15:10)。
> - 🧂 羅得紅綠燈(**新類型⑬忍住誘惑・向前跑**,`?demo=lotrun`,play-lotrun.bat):創 19:15-17+路 17:32;
>   按住往山上跑;誘惑跳出「👀 回頭看一眼」大鈕(千萬別按!);按了=鹽白化驚險一刻→天使拉住(19:16 憐恤)不算輸;
>   星等=忍住幾次;羅得妻子結局不畫,用路 17:32 帶教導;青檔誘惑中不能鬆手。
> - 🦁 各從其類・動物歸艙(**新類型⑭彈珠配對/泡泡龍反向化**,`?demo=arkmatch`,play-arkmatch.bat):創 6:20+7:8-9,14-16;
>   六角網格動物泡泡瞄準發射,同類 3+=「各從其類」一起進方舟(非爆破);懸空=神親自招聚;
>   全清=耶和華關門(7:16)+光;只發場上還有的種類防死局;永不會輸。
> - 全鏈:cuv 逐字 9 段(太 25:14-15/25:21/林前 4:2/路 15:8-10/創 19:15-17/路 17:32/創 6:20/7:8-9/7:14-16)→
>   引擎+Demo+路由+bat → mp3 烤畢(manifest 55 句)→ npm test → Playwright 截圖 → PR #74 待審。**37 引擎、39 bat**。

> ## ⭐ 對齊現況:2026-07-07(接手的 AI 先讀這段)
>
> **✅ 拍板兩關已做完(agape250 機,feat/temple-gideon,牧者已過審 2026-07-07、PR #73 併入 main):**
> - 🧱 活石蓋聖殿(**新類型⑩落石砌合**,`src/minigames/temple/`,`?demo=temple`,play-temple.bat):王上 6:7+彼前 2:5;
>   砌合發光非爆炸、堆頂「歇口氣」沉降不算輸、全程無鎚斧聲;年齡三檔=速度/預覽(2/1/0)/石形(3/5/7)。
> - ⚒️ 基甸拆祭壇(**新類型⑪打磚塊**,`src/minigames/gideon/`,`?demo=gideon`,play-gideon.bat):士 6:25-27 夜裡奉命拆巴力壇;
>   ⚠ 絕不可換皮成耶利哥;球掉出僕人撿回不扣命;拆完必接「築真壇」一幕(士 6:26);年齡三檔=球速/槓寬/磚層。
> - 全鏈已走:cuv 逐字查經(王上 6:7/彼前 2:5/士 6:25-27/約一 5:21)→ 引擎+Demo+modal 分支+路由+bat →
>   同輪烤曉臻 mp3(manifest 51 句,新產 2 → 重跑 0)→ **npm test 151 綠** → Playwright 三狀態截圖(console 僅 favicon 404)。
> - 現況:**33 引擎、35 bat、manifest 51 句**。✅ 已併 main(e52fe34);A 站驗證與大廳點卡進行中(2026-07-07)。

> ## ⭐ 對齊現況:2026-07-06 深夜(接手的 AI 先讀這段)
>
> **✅ 本日 agape250 機四批全上線(PR #69-72 皆牧者過審併入,大廳點卡):**
> - 🛡️ 尼希米守望(塔防⑤)+🕊️ 以斯拉護送(護送⑥)——PR #69(HFPC 機 WIP 尋回)
> - 🌱🦊💧 守護反應三式(打地鼠反向化⑦)——PR #70
> - ⚔️ 穿戴全副軍裝(換裝⑧)+🦊 小狐狸加難(洞 6/8/10、每波 1/2/3-4 隻)——PR #71
> - 🧺 摩西的籃子(縱向捲軸漂流⑨,雷電反向化)——PR #72
> - 現況:**31 引擎、33 bat、manifest 49 句、npm test 149 綠**;大廳 35 卡(sw v27)。
>
> **🔜 真正待做(HFPC 機從這裡開始;使用者已拍板):**
> 1. 🧱 活石蓋聖殿(俄羅斯方塊,temple/,王上 6:7+彼前 2:5;消行=砌合發光、堆頂不輸)~1天
> 2. ⚒️ 基甸拆祭壇(打磚塊,gideon/,士 6:25-27;⚠ 絕不可換皮成耶利哥)~1天
> 完整規格與守法紅線見 讀我-HANDOFF.txt ★段 與 合輯 roadmap 待做表前兩列。


> ## ⭐ 對齊現況：2026-07-06（最新，接手的 AI 先讀這段）
>
> **✅ 新類型③滑塊拼圖(2026-07-06,HFP 機)——🧥 約瑟的彩衣(創 37→50):**
...
> | 7 | 太鼓第二關：大衛迎約櫃(撒下6,只取歡慶段) | ~半天 | miriam 引擎換皮 |
> | 8 | 巴蘭/摩西動作版從 war-games 搬進 paul | 各~半天 | 舊待辦，自動部署受惠 |
> | 9 | 離線啟動選單頁、投影計分板 | 各~半天 | 舊待辦，高 CP |
>
> ---
>
> ## ⭐ 對齊現況：2026-06-18（晚）（歷史段）
>
> **本輪做完並上線（main 已併、Netlify 自動部署）：**
> - 🏆 **通關獎狀（PR #39）**：結束畫面**每位玩家**一顆「🏆 獎狀」鈕 → 下載「○○完成《○○》」PNG（`src/certificate.js`，羊皮紙金框 + 得勝印章 + 旅程名/頭銜/分數/日期）。純 Canvas、零美術檔、可離線/列印；**任何旅程通用**。
> - 📐 **桌遊內卡片關滿版（PR #40）**：牧師回報「桌遊內卡片關未全螢幕每列只 8 字、全螢幕才 21 字」。原因＝桌遊內走 `.modal__overlay` 置中小框 + 16:9 短舞台把字擠窄。修正：卡片關（`minigame.cards`）一律走 `.carddemo` 滿版（不分 `fill`）→ 未全螢幕也跟全螢幕一樣寬、一樣多字、**只剩一顆按鈕**（與聖歌/反轉動作版一致）+ 右上 ⛶；桌遊 Canvas 關（配對/蓋舟）維持原置中彈窗。`.carddemo` 加 `z-index:60` 才蓋得住棋盤。
>
> **本輪確認 / 結案：**
> - ✅ **實機驗全螢幕挖孔/置中**：牧者在實機（有挖孔的手機）按 ⛶ 進全螢幕確認 **OK**，不再黑邊、置中正常 → 此項**結案**，移出待做。
>
> **真正待做（CP 排序見 `docs/未來功能與skill點子-CP排序.md`）：**
> - 挪亞題庫/教導文案、彩虹卡片文案：**AI 草擬，請牧者用 `bible-content-reviewer` 過目**（已上線可隨時 revert）。
> - 巴蘭/摩西動作版仍在 war-games（手動部署）→ 可比照聖歌搬進 paul（`?demo=`），就能自動部署。
> - 離線啟動選單頁（老師一頁點開所有關卡）、投影計分板（各半天，高 CP）。
> - （選配）把通關獎狀抽成可重用 skill `award-certificate`。
>
> ---
>
...
> - ~~通關獎狀~~ ✅ 已上線（PR #39，見最上方 06-18 晚段）。離線啟動選單、投影計分板（各半天，高 CP）仍待做。
> - ~~實機驗：動作關全螢幕挖孔黑條、置中~~ ✅ 牧者實機確認 OK，結案。
>
> ---
>
...
>
> **✅ 已完成並上線**
> - **挪亞兩關**(arkpairs 一公一母配對+安排房間 / arkbuild 蓋方舟)已併 main + 大廳「挪亞方舟」合輯卡(PR #15、大廳 #4)。
> - **arkpairs 難度旋鈕**(輕鬆/普通/挑戰)+ ⭐星等 + 手機全螢幕無黑邊(PR #19)。
> - **聖歌(約沙法)+ 反轉(巴蘭) 卡片關升級 L6 手繪 Canvas、去 emoji**(PR #21)——系列**全部卡片關**現皆 L6 手繪(規矩:卡片關一律手繪 Canvas,不用 emoji 小劇場)。
> - **盼望·以利亞動作關**:遇天使後**空中撒餅🍞水💧、跳起來吃**才回體力 + 第4/5天使台詞精簡 + 說明對齊(PR #22/#24)。
> - **聖歌動作版搬進 paul**(`?demo=jehoshaphat-action`,**戰爭關永久家=paul**)+ 大廳改指向 paul + **敵軍放大~1.35×加眉毛嘴巴(三表情)**(PR #26、大廳 #5)。
> - **動作關手機全螢幕一條龍**(福音/大光/聖歌 + 大衛/以利亞/紅海/挪亞×2 共 8 關):16:9 取最大框滿版、**遊玩時隱藏標題列**(遊戲最大、米白邊最少)、**⛶ 全螢幕鈕 + 暫停鈕**、開場框不再被切、全螢幕挖孔安全區鋪暖底、去掉「開發預覽」字樣(PR #20/#23/#25/#27/#28)。心法已固化成 skill **`embed-fullscreen-fit`**。
>
> **🔧 真正待做(CP 排序見 `docs/未來功能與skill點子-CP排序.md`)**
> - **挖孔黑條**:`:root:fullscreen` 鋪底已上線,但 Playwright 無法模擬鏡頭挖孔 → **請在有挖孔的實機按 ⛶ 進全螢幕確認**;若還有黑邊改用 `env(safe-area-inset-*)` padding。
> - **巴蘭/摩西動作版仍在 war-games**(手動部署的原型站):本機無 netlify CLI(已裝在 HFP 機)。可比照聖歌搬進 paul(`?demo=`),就能自動部署。
> - **jonah `feat/redsea` 的聖歌敵人改動已 push 但 war-games 尚未重部署**(若仍用 war-games 入口才需要;大廳已改指向 paul,所以聖歌敵人新版**已隨 paul 上線**)。
> - **文案待牧者審**:盼望/大光卡片版、挪亞各關教導、聖歌/巴蘭卡片文案(併 main = 自動曝光,審完再說)。
> - **紅海放哪**的舊決策、通關獎狀、離線啟動選單、投影計分板 等——見 CP 排序文件。
>
> **🧠 重要慣例(踩過坑,別重犯)**
> - 部署一律**走 PR + `gh pr merge`**,不直推 main(classifier 會擋);本機 git 身分要設成 `summer09201017-cloud`。
> - 大廳連 paul `?demo=` 的卡片:**先合併並部署 paul,再合併大廳**,否則點了 404。
> - 卡片關一律 L6 手繪 Canvas(不用 emoji);動作關遊玩時隱藏標題列;改完零美術檔關卡用 Playwright 手機橫向截圖驗收。
> - 戰爭關(摩西/紅海/聖歌/巴蘭)永久家 = paul(消除跨 repo sync);`src/minigames/jonah/` 是約拿 copy,改它記得對齊上游避免 `sync:jonah` 漂移。
