...
**玩法內容**
- **七條旅程 + 旅程選單**（四條在 main、但以理/出埃及在 `feat/daniel-exodus`、海路到羅馬在 `feat/journey4-rome`）：
  ...
  - **🆕 海路到羅馬（2026-06-12 晚骨架完成，分支 `feat/journey4-rome`）**：**20 站**（該撒利亞上訴凱撒→西頓→每拉換船→佳澳→**友拉革羅狂風 L2 mustStop**（`cast:false`）→天使的應許→拋錨/逃船/擘餅→船擱淺→米利大毒蛇→部百流→敘拉古→部丟利→**亞比烏大道 L4 mustStop**→三館→羅馬「並沒有人禁止」），全地中海 gen-map region（義大利/馬耳他/克里特真實海岸線，aspect 1.659），機會卡 10/命運卡 8、RPG 層（路加/亞里達古/猶流同工＋四個錨 guard/擘餅/炭火/書卷 gifts），**宣教接力 `paul3 → paul4` 已接通＝完整保羅一生**。7 旅程 selfplay 全綠。⚠ 題卡全 AI 草擬，送審清單 `docs/題庫送審清單-journey4-2026-06-12.html`（在該分支），牧者過審才 merge。
...
0. **牧者審核 feature branch**：本 repo `feat/daniel-exodus`（但以理/出埃及，含卡片關文案）、本 repo `feat/journey4-rome`（海路到羅馬——⚠ 從 daniel-exodus 切出，先併它再併這支）、約拿 repo `feat/quiz-ch3-4`(題庫 3–4 章)、約拿 repo `art-kenney-sprites`（美術定案：只留木箱+老鼠）——實測滿意才 merge main(main 自動部署)。題目已納入「題庫送審清單」。（✅ 本 repo `feat/rpg-items-batch1` 道具第一批＝牧師本人設計的資料層，2026-06-12 下午已併入 main：validate＋selfplay＋煙霧全綠後推送，線上可實測，不滿意可 revert。）
...
2. ~~第三次宣教旅程~~ **✅ 已完成並上線（2026-06-11）**：`journey3.json` 20 站（以弗所三年事件群/推喇奴/士基瓦/焚書/底米丟/羅馬書/猶推古/米利都道別/亞迦布/耶路撒冷）+ 2 闖關站（高原 L4、海程 L2）+ RPG 層；`gen-map.mjs` region3 真實愛琴海地圖；宣教接力 `paul2 → paul3` 已接通；四旅程 selfplay 全綠。**✅ journey3 題目與文案 牧者審核通過（2026-06-11）。**
   ~~仍待做：「海路到羅馬」~~ **✅ 骨架已完成（2026-06-12 晚，分支 `feat/journey4-rome`）**：`journey4.json` 20 站 + 2 必停闖關（友拉革羅 L2 `cast:false`、亞比烏大道 L4）+ RPG 層 + 全地中海 gen-map region + `paul3 → paul4` 接力；7 旅程 selfplay/validate/煙霧/build 全綠。**剩：牧者審題**（清單在該分支 `docs/`）。⚠ 此分支從 `feat/daniel-exodus` 切出——先併 daniel-exodus 再併它才乾淨。
