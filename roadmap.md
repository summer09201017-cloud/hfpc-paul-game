...
2. **第三次宣教旅程 + 海路到羅馬**：用 `roll-and-move-game` skill 照 `journey2` 的方式新增 `journey3.json`（徒 18:23–21:17，以弗所三年…）與「到羅馬」（徒 27–28，海難）。城市座標：在 `gen-map.mjs` 加一個 region（或沿用第二次的範圍）再 `npm run gen:map`；最後把它加進 `useGame.js` 的 `JOURNEYS`。記得在 `JOURNEYS` 把 `paul2` 的 `nextKey` 接到 `paul3`（宣教接力）。
