// ===========================================================================
// sync-petersea-engine — 一鍵把「彼得走海」節奏關引擎同步進保羅大富翁的嵌入小遊戲
// ---------------------------------------------------------------------------
// 彼得走海(hfpc-peter-sea-game)是即時 vanilla Canvas 節奏關(太14 定睛看耶穌走海面);
// 保羅大富翁把它當「耶穌生平之旅④水面行走」站(?journey=jesus)的挑戰關嵌入。
// src/minigames/paulsilas/ 是彼得走海引擎的一份 copy。獨立版那邊改完(加經文出處
...
//
// 用法:
//   node scripts/sync-petersea-engine.mjs                 (自動尋找彼得走海 src)
//   node scripts/sync-petersea-engine.mjs --from=<彼得走海的 src 路徑>
//   PETERSEA_SRC=<彼得走海的 src 路徑> node scripts/sync-petersea-engine.mjs
//   node scripts/sync-petersea-engine.mjs --check         (只報告會變動什麼,不寫檔)
//
...
const DEST_DIR = path.join(PROJECT_ROOT, 'src', 'minigames', 'petersea')
import('node:fs').then(m=>m.mkdirSync(DEST_DIR,{recursive:true}))

// 從這個檔開始追 import(保羅的 MiniGameModal 就是 import 它的 Game)。
const ENTRY = 'game.js'
// 永遠不要複製過來的「app 外殼」(嵌入由 React MiniGameModal / PeterSeaDemo 取代)。
...
// --- 找出彼得走海的 src 目錄 ---------------------------------------------
function resolvePeterSeaSrc() {
  const candidates = []
  if (fromArg) candidates.push(fromArg.slice('--from='.length))
  if (process.env.PETERSEA_SRC) candidates.push(process.env.PETERSEA_SRC)
  // 本團隊常見擺法:獨立版放在桌面「彼得走海」
  candidates.push(path.join(os.homedir(), 'Desktop', '彼得走海', 'src'))
  // 或當成 paul-game 的姊妹 repo
  candidates.push(path.resolve(PROJECT_ROOT, '..', 'hfpc-peter-sea-game', 'src'))
  candidates.push(path.resolve(PROJECT_ROOT, '..', '..', 'hfpc-peter-sea-game', 'src'))
  ...
  console.error('✗ 找不到彼得走海引擎的 src 目錄(裡面要有 game.js)。試過:')
  for (const c of candidates) if (c) console.error('    ' + path.resolve(c))
  console.error('  請用 --from=<彼得走海的 src 路徑> 或設環境變數 PETERSEA_SRC 指定。')
  process.exit(1)
}
...
function collectModules(srcDir) {
  ...
  while (queue.length) {
    ...
    if (!existsSync(full)) {
      console.error(`✗ ${file} 被 import 卻不存在於彼得走海 src:${full}`)
      process.exit(1)
    }
    ...
    for (const m of code.matchAll(LOCAL_IMPORT)) {
      ...
      if (NEVER_COPY.has(dep)) {
        ...
        console.error('  彼得走海引擎必須「ui 畫在 Canvas + 由 MiniGameModal 注入」而非 import 外殼,')
...
// --- 主流程 --------------------------------------------------------------
const SRC = resolvePeterSeaSrc()
console.log(`彼得走海引擎 src:${SRC}`)
