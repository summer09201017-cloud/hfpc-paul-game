...
// ===========================================================================
// 彼得跟隨主的一生（真實地理版）——聖地：加利利海 → 該撒利亞腓立比（極北）→
//   耶路撒冷（南）→ 回加利利海邊 → 耶路撒冷 → 該撒利亞（海邊）。
//   ⚠ 同一地有多個事件（加利利海邊好幾關、耶路撒冷一帶好幾關）——為了在棋盤上各自看得清，
//   把同地事件「沿真實的湖岸／繞耶路撒冷一帶」微幅錯開成鄰近但不同的點（地圖可放大看細節）。
//   路線會在加利利↔耶路撒冷之間來回——這是真實的：主的事工本來就往返兩地。
// ===========================================================================
buildRegion({
  label: '彼得跟隨主的一生',
  bounds: { lonMin: 34.6, lonMax: 35.95, latMin: 31.45, latMax: 33.45 },
  // 以色列/巴勒斯坦為主，北含黎巴嫩/敘利亞（黑門山下該撒利亞腓立比），東含約旦（約旦河東）
  iso: ['ISR', 'PSE', 'LBN', 'SYR', 'JOR'],
  journeyFile: 'journey-peter.json',
  mapFile: 'region-map-peter.json',
  labels: [
  ...
  cities: {
    // —— 加利利海一帶（蒙召與事工；沿湖岸錯開）——
    calling_lake:       { lat: 32.83, lon: 35.50 }, // 加利利海邊蒙召（革尼撒勒西岸）
    capernaum:          { lat: 32.90, lon: 35.575 }, // 迦百農（北岸，彼得的家）
    great_catch:        { lat: 32.86, lon: 35.655 }, // 下網得魚（湖面東）
    sermon_mount:       { lat: 32.945, lon: 35.53 }, // 山上聽道（北面山地，八福山）
    storm_calmed:       { lat: 32.755, lon: 35.61 }, // 海上的風浪（湖中偏南）
    feeding_5000:       { lat: 32.915, lon: 35.645 }, // 五餅二魚（東北岸伯賽大一帶）
    walk_on_water:      { lat: 32.81, lon: 35.625 }, // 水面上行走（湖面中央）
    forgive_seventy:    { lat: 32.875, lon: 35.45 }, // 饒恕七十個七次（迦百農西）
    transfiguration:    { lat: 32.69, lon: 35.39 }, // 變像山（他泊山，湖西南）
    // —— 該撒利亞腓立比（極北，黑門山下）——
    great_confession:   { lat: 33.28, lon: 35.69 }, // 該撒利亞腓立比的大認信
    // —— 受難週：耶路撒冷一帶（繞城錯開；橄欖山事件偏東）——
    washing_feet:       { lat: 31.84, lon: 35.10 }, // 主為門徒洗腳（樓上，城西）
    last_supper:        { lat: 31.81, lon: 35.135 }, // 最後晚餐 · 預言不認
    gethsemane:         { lat: 31.82, lon: 35.27 }, // 客西馬尼園（橄欖山，城東）
    denial:             { lat: 31.78, lon: 35.18 }, // 三次不認主（大祭司院子）
    empty_tomb:         { lat: 31.75, lon: 35.22 }, // 空墳墓
    // —— 復活後：回加利利海邊 ——
    restoration:        { lat: 32.745, lon: 35.51 }, // 提比哩亞海邊 · 你愛我嗎（西南岸）
    // —— 回耶路撒冷：升天與五旬節 ——
    ascension:          { lat: 31.79, lon: 35.31 }, // 主升天（橄欖山／伯大尼，城東）
    pentecost:          { lat: 31.72, lon: 35.205 }, // 五旬節 · 聖靈降臨
    council_witness:    { lat: 31.70, lon: 35.27 }, // 美門口醫病 · 公會作見證（聖殿一帶）
    // —— 福音出耶路撒冷，到外邦（海邊）——
