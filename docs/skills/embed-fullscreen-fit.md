...
### 7) 純 React 卡片關「相反處理」——要滿版可捲動高卡片,不要鎖比例、不要置中小彈窗
固定比例 Canvas 關(#1)要鎖 16:9;但**純 React 的卡片流程關(內容是大字 + 選項 + 經文,會變高)正好相反**:
- ❌ 別把卡片塞進 16:9 舞台:寬而矮的框會把長文字擠成窄窄一條(手機上**每列只剩 ~8 個字**),且下半被裁。
- ❌ 別用「置中小彈窗」(`.modal__overlay` + `max-width:560px`):桌遊內嵌時字一樣窄,且沒有 ⛶。
- ✅ 卡片關一律走**滿版外框**(`position:fixed; inset:0`、淺底)、內層一張 `max-width:min(1100px,95vw)` 的**可捲動高卡片**、投影大字(`clamp` 字級)。這樣**未全螢幕也跟全螢幕一樣寬**(每列 ~21 字),只剩「開始/下一步」一顆按鈕,並附 ⛶。
- ⚠ 同一個宿主元件若同時服務 Canvas 關與卡片關(如 paul 的 `MiniGameModal`):**用「是不是卡片關」決定外框,而不是用「是否單獨玩(fill)」**。否則桌遊內嵌的卡片關會落回置中小彈窗(paul 2026-06-18 修過:`cardSpec ? '.carddemo' : fill ? '.carddemo--game' : '.modal__overlay'`)。
- ⚠ 滿版外框在**桌遊內**要蓋住既有畫面 → 記得給夠高的 `z-index`(paul `.carddemo` 設 60,蓋過棋盤面板 8 與彈窗 50);單獨玩 `?demo=` 時無害。
...
- ❌ 把純 React 卡片關當成 Canvas 關鎖進 16:9 舞台、或塞進置中小彈窗 → 文字被擠成每列 ~8 字、玩家難讀(#7;paul 桌遊內卡片關 2026-06-18 修過,改走滿版 `.carddemo`)。
