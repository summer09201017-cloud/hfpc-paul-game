...
export default function MiniGameModal({ minigame, onComplete, fill = false }) {
  ...
  const begin = () => {
    ...
    if (isSower || isFoxes || isSparks || isArmor || isBasket || isTemple || isGideon || isSteward || isLostCoin || isLotRun || isArkMatch || isFragments || isFruits || isGems || isHerd || isGoalKick || isManna || isGlean || isFlock) {
      // 守護反應三式+換裝+漂流+砌合+打磚塊+管家+找物+忍誘惑+彈珠配對三關+歸位+撞球兩關+射門+嗎哪+拾穗:同一套嵌入契約(constructor 同形),依 engine 選類
      const Cls = isSower ? SowerGame : isFoxes ? FoxesGame : isSparks ? SparksGame : isArmor ? ArmorGame : isBasket ? BasketGame : isTemple ? TempleGame : isGideon ? GideonGame : isSteward ? StewardGame : isLostCoin ? LostCoinGame : isLotRun ? LotRunGame : isArkMatch ? ArkMatchGame : isFragments ? FragmentsGame : isFruits ? FruitsGame : isGems ? GemsGame : isHerd ? HerdGame : isGoalKick ? GoalKickGame : isManna ? MannaGame : isGlean ? GleanGame : FlockGame
