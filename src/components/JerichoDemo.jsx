...
import { Game as JerichoGame } from '../minigames/jericho/game'
import { AGE, SCRIPTURE } from '../minigames/jericho/config'
import { initSpeech, speakText } from '../speak'
import { getAgePref, setAgePref } from '../agePrefs'

// 耶利哥城牆(?demo=jericho,書 6)：忿怒鳥式——蓄力吶喊/吹角震塌城牆。
// ★神學:城牆是「耶和華使它塌陷」(書 6:20),玩家只管順服呼喊;沿用 slingshot 物理引擎換皮。
export default function JerichoDemo() {
  ...
  const begin = () => {
    ...
    const g = new JerichoGame(canvasRef.current, { embed: true, winPoints: 5, age, onComplete: (r) => setResult(r) })
    gameRef.current = g
    if (typeof window !== 'undefined') window.__jericho = g
    g.boot()
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#0f1922', display: 'flex', flexDirection: 'column' }}>
      ...
      {(!started || result) && (
        <div style={{ color: '#cfe3e8', padding: '6px 12px', font: '14px system-ui' }}>
          🎺 耶利哥城牆(書 6){result && `　→ ${result.won ? '城牆塌陷 🎺' : '吶喊用完'}（score ${result.score}）`}
        </div>
      )}
      <div style={{ position: 'relative', flex: 1, minHeight: 0 }}>
        ...
        {!started && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 16 }}>
            <div style={{ color: '#fff', font: 'bold 22px system-ui' }}>🎺 耶利哥城牆 · 第七天的呼喊　選一個年齡</div>
                ...
                return (
                  <button key={a.id} onClick={() => pickAge(a.id)}
                    style={{ width: 200, padding: '14px 16px', borderRadius: 14, cursor: 'pointer', textAlign: 'left',
                      ...
                      background: on ? 'rgba(202,140,60,0.92)' : 'rgba(20,30,40,0.7)', color: '#fff' }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
              ...
              <button onClick={begin} style={{ padding: '12px 28px', fontSize: 18, borderRadius: 12, border: 'none', background: '#c98a2b', color: '#fff', cursor: 'pointer', fontWeight: 'bold' }}>開始 →</button>
            </div>
          </div>
        )}
        {result && (
