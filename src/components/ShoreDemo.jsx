...
import { Game as ShoreGame } from '../minigames/shore/game'
import { AGE, SCRIPTURE } from '../minigames/shore/config'
import { initSpeech, speakText } from '../speak'
import { getAgePref, setAgePref } from '../agePrefs'

// 海邊的復興(?demo=shore,約 21:15-19)：耶穌三次問「你愛我嗎?」+ 三次託付「餵養我的羊」。
// ★神學:彼得三次不認主 → 主在炭火旁三次接納、託付 →「你跟從我吧」。⛔ 不渲染殉道。
export default function ShoreDemo() {
  ...
  const begin = () => {
    ...
    const g = new ShoreGame(canvasRef.current, { embed: true, winPoints: 5, age, onComplete: (r) => setResult(r) })
    gameRef.current = g
    if (typeof window !== 'undefined') window.__shore = g
    g.boot()
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#10202c', display: 'flex', flexDirection: 'column' }}>
      ...
      {(!started || result) && (
        <div style={{ color: '#cfe3e8', padding: '6px 12px', font: '14px system-ui' }}>
          🔥 海邊的復興(約 21){result && `　→ ${result.won ? '三次託付・跟從主 ❤️' : ''}`}
        </div>
      )}
      <div style={{ position: 'relative', flex: 1, minHeight: 0 }}>
        ...
        {!started && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 16 }}>
            <div style={{ color: '#fff', font: 'bold 22px system-ui' }}>🔥 海邊的復興 · 你愛我嗎?　選一個年齡</div>
                ...
                return (
                  <button key={a.id} onClick={() => pickAge(a.id)}
                    style={{ width: 200, padding: '14px 16px', borderRadius: 14, cursor: 'pointer', textAlign: 'left',
                      ...
                      background: on ? 'rgba(196,130,46,0.92)' : 'rgba(20,30,40,0.7)', color: '#fff' }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
              ...
              <button onClick={begin} style={{ padding: '12px 28px', fontSize: 18, borderRadius: 12, border: 'none', background: '#c4822e', color: '#fff', cursor: 'pointer', fontWeight: 'bold' }}>開始 →</button>
            </div>
          </div>
        )}
        {result && (
