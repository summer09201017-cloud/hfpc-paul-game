...
import { Game as NehemiahGame } from '../minigames/nehemiah/game'
import { AGE } from '../minigames/nehemiah/config'
import { CONTENT } from '../minigames/nehemiah/content'
import { initSpeech, speakText } from '../speak'
import { getAgePref, setAgePref } from '../agePrefs'

// 尼希米修牆・躲敵人的攻擊(?demo=nehemiah,尼 4、6)。掃羅閃避引擎換皮 + 年齡旋鈕 + 語音玩法。
// 反向 RPG:工人不還手,邊閃避邊把城牆建起來,「我們的神必為我們爭戰」(尼 4:20)。
export default function NehemiahDemo() {
  ...
  const begin = () => {
    ...
    setStarted(true)
    setResult(null)
    const g = new NehemiahGame(canvasRef.current, {
      embed: true,
      winPoints: 5,
      age,
    ...
    gameRef.current = g
    if (typeof window !== 'undefined') window.__nehemiah = g
    g.boot()
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#15110c', display: 'flex', flexDirection: 'column' }}>
      ...
      {(!started || result) && (
        <div style={{ color: '#ffe9b8', padding: '6px 12px', font: '14px system-ui' }}>
          🧱 尼希米修牆{result && `　→ 結果：${result.won ? '城牆修完了 🧱' : '修牆被攔阻了'}（score ${result.score}）`}
        </div>
      )}
      <div style={{ position: 'relative', flex: 1, minHeight: 0 }}>
        ...
        {!started && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: 16 }}>
            <div style={{ color: '#fff', font: 'bold 22px system-ui' }}>🧱 尼希米修牆・躲敵人的攻擊　選一個年齡</div>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: on ? 'rgba(122,64,48,0.92)' : 'rgba(30,22,14,0.7)',
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
              ...
              <button
                onClick={begin}
                style={{ padding: '12px 28px', fontSize: 18, borderRadius: 12, border: 'none', background: '#7a4030', color: '#fff', cursor: 'pointer', fontWeight: 'bold' }}
              >
                開始修牆 →
              </button>
            </div>
            <div style={{ color: '#cdab74', font: '12px system-ui' }}>← → 或左右半邊畫面移動工人；看準紅色預警閃開箭和石頭</div>
          </div>
        )}
