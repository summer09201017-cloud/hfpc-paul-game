// 尼希米修牆(尼 4、6)——掃羅閃避引擎換皮。嵌入契約:new Game(canvas,{embed,onComplete,age})、boot()、destroy()。
// 仇敵放箭/丟石,工人邊閃避邊把城牆建起來(躲過一波=牆升一段)。
// 狀態:intro →(確認)dodge →(城牆修完)win ／(被打中超過上限)lose。靠神得勝,不還手。
...
export class Game {
  ...
  _finish() {
    ...
    this.onComplete({ won, score: won ? this.winPoints : 0, level: 'nehemiah' })
  }
