...
// （2026-06-27 升級成「拖曳彈弓」後 sweep 不再用於瞄準；保留欄位給舊參考/不刪以免別處引用出錯。）
export const AIM = {
  minDeg: 0,
  maxDeg: 90,
  sweepDegPerSec: 44,
...
// 拖曳彈弓（忿怒鳥式）：按住機弦往後拉設「角度+力道」，放手射出。發射速度 = 拉的向量 × power，夾 min/max。
// anchor = 機弦脫手點（≈大衛肩/手）；往後拉(左下) → 朝歌利亞(右上)射出。
export const DRAG = {
  anchorX: 168, anchorY: 354, // 機弦脫手點（≈大衛肩;= DAVID.x, DAVID.y-6）
  power: 7.6, // 拉距換算速度倍率（滿拉 maxPull×power ≈ 912，夠射到歌利亞）
  maxPull: 120, // 最大拉距(px)
  maxSpeed: 1050, // 發射速度上限
  minPull: 16, // 拉不到這個距離放手＝視為誤觸，不發射、不浪費石子
}
...
export const AGE = {
  ...
  kids:   { id: 'kids',   label: '兒童',   emoji: '🙂', sub: '一般玩法（7–12 歲）｜歌利亞會左右移動',     sweepDegPerSec: 44, forehead: { x: 792, y: 172, w: 28, h: 18 }, stones: 3, speakHowto: false, timed: false,
