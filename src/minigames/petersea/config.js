...
export const DIFF = {
  ...
  // 🔥 地獄：更快、判定極嚴、漏拍重摔。仍 canSink + 無限救援（硬但不絕望，合系列調性）。
  // slowMul：到「密集慢板段」(chart.slow 時間窗，約 40% 進度) 捲動降到這個倍率 → 音符擠成一疊慢慢爬，考驗觀察力（判定時間不變）。
  hell: {
    bpm: 132,
    scroll: 560,
    win: { perfect: 0.055, good: 0.10, bad: 0.15 },
    faithStart: 0.45,
    gain: { perfect: 0.04, good: 0.025, bad: 0.012 },
    missPenalty: 0.13,
    floor: 0,
    canSink: true,
    slowMul: 0.45,
...
// 拉長音符（hold / 持守音）：按住期間每秒加的信心、放太早的破壞懲罰、結尾寬限、尾巴寬度
export const HOLD = { gainPerSec: 0.10, breakPenalty: 0.05, grace: 0.12, tailW: 26 }
