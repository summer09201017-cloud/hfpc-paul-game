...
import { VIEW, HIGHWAY, DIFF, SCORE, STARS, COUNT_BEATS, RESCUE as RCFG, FAITH, HOLD } from './config.js'
...
export class Game {
  constructor(canvas, opts = {}) {
    ...
    this.buttons = {
      start: { x: 350, y: 392, w: 260, h: 64, label: '踏出船 ▶' },
      walk: { x: 36, y: 150, w: 288, h: 232 },
      run: { x: 336, y: 150, w: 288, h: 232 },
      hell: { x: 636, y: 150, w: 288, h: 232 },
      again: { x: 298, y: 448, w: 204, h: 44, label: '🔁 再走一次' },
  ...
  _stepPlay(dt) {
    ...
    // 密集慢板段：捲動平滑放慢（判定時間不受影響，音符會擠成一疊慢慢爬）
    let scTarget = this.scrollBase
    this.slowActive = false
    const sw = this.chart.slow
    if (sw && this.diff.slowMul && this.songPos >= sw.from && this.songPos <= sw.to) {
      scTarget = this.scrollBase * this.diff.slowMul; this.slowActive = true
    }
    this.scroll += (scTarget - this.scroll) * Math.min(1, dt * 4)
  ...
  _judgeLane(lane, t) {
    ...
    best.hit = true; best.anim = 1
    if (best.hold > 0) { best.held = true; best.holdStart = this.songPos }  // 拉長音符：命中頭、開始按住（judged 等撐完才設）
  ...
  // 拉長音符：已命中頭、尚未結算的，每幀檢查該軌是否仍按住
  _stepHolds(dt) {
    for (const n of this.chart.notes) {
      if (!n.hit || n.hold <= 0 || n.judged) continue
      const endT = n.time + n.hold
      if (this._laneHeld(n.lane)) {
        n.held = true
        this.faith = Math.min(1, this.faith + HOLD.gainPerSec * dt)
        if (this.recvFlash[n.lane] < 0.6) this.recvFlash[n.lane] = 0.6   // 接收圈持續發光
        if (this.songPos >= endT) {            // 撐到底 → 持守完成
          n.judged = true; n.holdDone = true
          this.combo++; this.maxCombo = Math.max(this.maxCombo, this.combo); this.comboPop = 1
          this.score += SCORE.good
          this._setJudge('持守!', '#9fe8cf'); this.audio.hit(0); this._splash(0)
        }
      } else if (this.songPos >= endT - HOLD.grace) {
        n.judged = true; n.holdDone = true     // 接近尾端才放 → 算過
      } else {                                  // 放太早 → 斷了
        n.judged = true; n.broke = true; n.held = false
        this.combo = 0
        this.faith = Math.max(this.diff.floor, this.faith - HOLD.breakPenalty)
        this._setJudge('放開了…', '#e8a83f'); this.audio.miss()
      }
    }
  }

  // 該軌是否被按住（鍵盤 held，或觸控指標壓在該軌）
  _laneHeld(lane) {
    if (this.input.held[lane]) return true
    const p = this.input.pointer
    if (p && p.down) { const L = this._toLogical(p.x, p.y); if (this._laneAt(L.x) === lane) return true }
    return false
  }
  ...
  _splash(q) {
    ...
    const fx = PETER_X + this.progress * 584   // 與 renderer _peter 同步(JESUS_X 824 - PETER_X 156 - 84)
  ...
  _resume() {
    ...
    for (const n of this.chart.notes) if (!n.judged && n.time <= sp + this.diff.win.bad) { n.judged = true; n.result = null; if (n.hold > 0) n.holdDone = true }
    this.state = 'play'
  }
  ...
  _pointers() {
    ...
    for (const p of this.input.drainPointers()) {
      ...
      if (this.state === 'title') { this.audio.unlock(); this.storyIndex = 0; this.state = 'story' }
        ...
        else if (inRect(L, this.buttons.hell)) { this.audio.unlock(); this._startCountdown('hell') }
      } else if (this.state === 'win') {
        if (inRect(L, this.buttons.again)) this._restart()
        else if (inRect(L, this.buttons.listen)) speakScripture(`${this.WIN.head}。${this.WIN.verse}`, { isMuted: () => this.muted, ref: this.WIN.ref })
  ...
  _hitButton(x, y) {
    ...
    if (this.state === 'mode') { if (inRect({ x, y }, B.walk)) return 'walk'; if (inRect({ x, y }, B.run)) return 'run'; if (inRect({ x, y }, B.hell)) return 'hell' }
  ...
  _startCountdown(mode) {
    ...
    this.scrollBase = this.diff.scroll
  ...
  _win() {
    ...
    this.progress = 1
    this.state = 'win'
    speakScripture(`${this.WIN.head}。${this.WIN.verse}`, { isMuted: () => this.muted, ref: this.WIN.ref })
