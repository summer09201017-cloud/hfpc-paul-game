...
> 卡片奇兵全升級成**手繪 Canvas 動畫**、有**背景音樂**與**奇兵關 3 條命（會輸）**。
> 盼望/大光的**卡片版**（`?demo=elijah`／`?demo=saul`）文案仍待牧者審、未連結。
> 🌊 紅海奔逃動作關（出 14）已做好、待決定放法（見 `讀我-HANDOFF.txt`「開放決策」）。
> **完整現況、奇兵卡片/動作矩陣、CP 排序 todo 見 `讀我-HANDOFF.txt`（2026-06-15 早）與 `roadmap.md`。**
...
**分支上、待併 main（見 `讀我-HANDOFF.txt`）**：`?journey=` 深連結（大廳卡片要它）、手機放大全白修正＋安裝鈕、首頁分類卡片選單、🆕 **大衛甩石「拋射引擎」**（`src/minigames/sling/`，可重用於未來投擲關，`?demo=sling` 預覽）、🛕 **挪亞方舟兩關**（`feat/noah-minigames` / PR #15）：`?demo=arkpairs`（一公一母翻牌配對＋安排房間解謎，母的戴 🎀、有背景音樂）、`?demo=arkbuild`（操作挪亞鎚擊放木板、旁人嘲笑）；大廳已加「挪亞方舟」合輯卡，待合併部署後連結才會通。
